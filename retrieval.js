...
function parseTestQuestions(testFilePath) {
  const text = fs.readFileSync(testFilePath, "utf8");
  const lines = text.split(/\r?\n/);
  const questions = [];

  lines.forEach((line) => {
    const match = line.match(/^\s*- Student Question:\s*(.+)$/);
    if (match) {
...
if (require.main === module) {
  ...
  const testFilePath = path.join(KNOWLEDGE_FOLDER, "retrieval_test_cases.md");
  const testQuestions = parseTestQuestions(testFilePath);

  console.log("Loaded", chunks.length, "knowledge chunks.\n");
  testQuestions.forEach((question, index) => {
    const results = findRelevantChunks(question, chunks);
    console.log(`Test ${index + 1}: ${question}`);
    results.forEach((result, rank) => {
      console.log(`  ${rank + 1}. ${result.chunk_id} (${result.topic}) score=${result.relevance_score}`);
      console.log(`     matched: ${result.matched_keywords.join(", ")}`);
    });
    if (results.length === 0) {
